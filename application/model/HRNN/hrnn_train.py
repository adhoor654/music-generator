...
# 	--data_dir=AUDIO/train \
# 	--val_data_dir=AUDIO/val \
# 	--big_frame_size=16 \
# 	--frame_size=16 \
# 	--rnn_type=LSTM \
# 	--dim=256 \
...
from model import InputHandler
...
def main():
    ...
    ####define graph####
    net = HRNN(if_train = True, args = args)
    ...
    with tf.name_scope('create_inputs'):
        reader = InputHandler(coord =coord,
                             args = args)
    ...
    try:
        if args.mode_choice=="ad_rm2t_fc":
            ...
            for step in range(saved_global_step + 1, args.num_steps):
                ...
                X, y, remaining_time = sess.run(audio_batch)
                ...
                ground_truth_train, prediction_train, summary_train, loss_train, _ = \
                    sess.run(outp_list_train, feed_dict=inp_dict_train)
                ## run test op ##
                if step % 500 == 0:
                    ground_truth_test, prediction_test, summary_test, loss_test = \
                        sess.run(outp_list_test, feed_dict=inp_dict_test)
