...
class HRNN(object):
    ...
    def bln_attn(self, baseline_input, baseline_state = None, if_attn = True):
        ...
        with tf.variable_scope("baseline"):
            if baseline_state is None: #during training
                bln_outputs_all, baseline_last_state = tf.nn.dynamic_rnn(cell, baseline_input, dtype=tf.float32)
        ...
        return rhythm_logits, bar_logits, note_logits, baseline_last_state
    ...
    def loss_HRNN(self, X,y, rm_time = None,l2_regularization_strength=None, name='sample'):
        ...
        with tf.name_scope(name):
            if self.mode_choice=="ad_rm2t_fc":  #2 tier with acc time
                pd_sustain, pd_bar, pd_note = self._create_network_ad_rm2t_fc_tweek_last_layer(one_t_input = self.X, rm_tm = self.rm_time)
